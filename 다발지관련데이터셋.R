...
dabalji_subset <- function(git_path="C:/Users/user/Documents/GitHub/No_namedTeam") {
  
  #accident 주데이터와 결합
  accident <- read.csv(paste(git_path, "Kor_Train_교통사망사고정보(12.1~17.6).csv", sep="/"))
  mudanGo <- read.csv(paste(git_path, "04.무단횡단사고다발지/무단횡단사고다발지(2012~2016).csv", sep="/"))
  olderGo <- read.csv(paste(git_path, "05.보행노인사고다발지/보행노인사고다발지(2012~2016).csv", sep="/"))
  childGo <- read.csv(paste(git_path, "06.보행어린이사고다발지/보행어린이사고다발지(2012~2016).csv", sep="/"))
  schoolZoneGo <- read.csv(paste(git_path, "07.스쿨존내사고다발지/스쿨존내어린이사고다발지(2012~2016).csv", sep="/"))
  cycleGo <- read.csv(paste(git_path, "08.자전거사고다발지/자전거사고다발지(2012~2016).csv", sep="/"))
  ...
  size = as.integer(nrow(accident)*0.85)
  train <- accident[1:size,]
  test <- accident[(size+1):nrow(accident), ]
  ...
  #데이터 train에만 보조데이터셋을 붙이기
  temp <- left_join(train, total_dabalji_dataSet, by = c("발생지시군구", "발생지시도", "발생년"))
